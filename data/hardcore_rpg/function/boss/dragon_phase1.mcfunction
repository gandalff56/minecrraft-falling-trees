...
execute if predicate hardcore_rpg:boss_summon_chance in minecraft:the_end as @a[scores={rpg.level=30},limit=1,sort=random] at @s run summon minecraft:enderman ~5 ~ ~5 {custom_name:{"text":"Straznik Smoka","color":"dark_purple","italic":true},Tags:["hrpg.dragon_minion","hrpg.scaled","hrpg.elite_checked"]}
