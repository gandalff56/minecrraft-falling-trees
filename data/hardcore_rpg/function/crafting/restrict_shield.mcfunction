...
execute if score @s rpg.temp matches 1.. run tellraw @s ["",{"text":"✖ ","color":"red"},{"text":"Tylko ","color":"gray"},{"text":"Warrior","color":"red","bold":true},{"text":" może używać tarczy!","color":"gray"}]
execute if score @s rpg.temp matches 1.. run playsound minecraft:block.anvil.land player @s ~ ~ ~ 0.5 2
execute if score @s rpg.temp matches 1.. run title @s actionbar {"text":"⚠ Tarcza tylko dla Warriora!","color":"red"}
