scoreboard players set #raycast main_score 2
execute unless block ~ ~ ~ #physics_of_falling_trees:air align y run function physics_of_falling_trees:tree/falling/leaves/loop
execute if score #raycast main_score matches 0 align z run tp ~ ~ ~-0.1
execute if score #raycast main_score matches 1 align y run tp ~ ~1 ~
