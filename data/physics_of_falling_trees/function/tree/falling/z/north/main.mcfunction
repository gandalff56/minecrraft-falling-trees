...
execute as @e[type=#physics_of_falling_trees:tree_displays,limit=800,tag=!poft.ignor_offset,tag=!poft.main_block,tag=poft.block,predicate=physics_of_falling_trees:same_ids] run function physics_of_falling_trees:tree/falling/z/north/offset

execute if score @s[tag=!poft.moved] poft.angle matches 90001.. run function physics_of_falling_trees:tree/falling/z/north/tp